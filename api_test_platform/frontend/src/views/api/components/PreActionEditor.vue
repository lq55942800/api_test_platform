<template>
  <div class="pre-action-editor">
    <div class="action-toolbar">
      <el-dropdown trigger="click" @command="handleAdd">
        <el-button type="primary" link size="small">
          <el-icon style="margin-right: 4px"><Plus /></el-icon>添加前置操作
        </el-button>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item v-for="at in PRE_ACTION_TYPES" :key="at.value" :command="at.value">
              {{ at.label }}
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
...
import { PRE_ACTION_TYPES } from '@/types/api'
import type { PreRequestAction, PreRequestActionType } from '@/types/api'
...
const props = defineProps<{
  modelValue: PreRequestAction[]
}>()

const emit = defineEmits<{
  'update:modelValue': [value: PreRequestAction[]]
}>()

const actions = ref<PreRequestAction[]>([...(props.modelValue || [])])
...
function updateConfig(action: PreRequestAction, config: Record<string, any>) {
...
.action-order { width: 18px; height: 18px; border-radius: 50%; background-color: #aa3bff; color: #fff; font-size: 11px; font-weight: 600; display: flex; align-items: center; justify-content: center; }
